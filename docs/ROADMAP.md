...
- 输入：主界面照片库中的单个或多个视频；iCloud 项目仅在用户点选后下载；
...
1. 建立 iOS 26 公开 `kVTCompressionPropertyKey_*` 目录；
2. 按当前编码类型、素材分辨率和硬件编码器读取支持字典；
3. 为 Boolean、Number、Enumeration、Array 与 Dictionary 等公开类型生成对应编辑器；
4. 本机只读或不支持的字段保留可见但置灰；
5. 码率控制、VBV 和多遍／实时等互斥字段只显示当前有效分支；
6. MultiPassStorage 使用原生多遍流程；设备准备阶段拒绝时透明回退单遍并记录证据。

#### 约束

- 复杂值仅允许编辑公开目录中已知字段的 JSON 表示，不允许添加任意键或私有属性；
- 每个控件必须绑定明确的公开属性、数据类型、允许范围、设置时机和互斥规则；
- 设置成功后必须回读；转码结束后必须重新读取输出；
- 不把 CRF（Constant Rate Factor，恒定质量因子）伪装成 VideoToolbox 原生参数；
- 不把模板、源码率比例、峰值倍数或“编码质量档位”等 App 逻辑伪装成原生字段。
