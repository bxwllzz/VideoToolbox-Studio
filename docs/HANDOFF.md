...
1. 从 Files 选择本地 MOV 或 MP4；
2. 读取视频轨道、时间戳、方向、编码格式和色彩信息；
3. 使用 H.264 或 HEVC 硬件路径编码，输出固定为 MOV；
4. 兼容音频时直通，否则转 AAC；
5. 显示进度并支持取消，失败或取消后清理不完整文件；
6. 重新打开输出文件，核验时长、方向、音画同步、Codec 和实际格式；
7. 输出文件与版本化报告均可通过 Files 分享；
8. 用仓库内无版权测试素材完成 CI，再用云真机自动回归。
