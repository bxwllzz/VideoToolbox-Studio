...
> 更新日期：2026-07-25
> 当前阶段：P1 空壳 App 与安装闭环实施中
> 当前路线：免费 Apple Account + SideStore 首次电脑初始化 + GitHub Actions 云端构建 + iPhone 真机验证
...
在用户完成 P0 并提供仓库后，只实现以下内容：

1. 创建最小 SwiftUI iPhone 工程；
2. 使用 XcodeGen 管理工程；
3. 固定可用的 macOS runner、Xcode 和 XcodeGen 版本；
4. 建立一个持续集成工作流，完成编译和单元测试；
5. 建立一个手动触发的未签名 IPA 打包工作流；
6. 首页显示：
   - App 版本与构建号；
   - Git commit SHA（Secure Hash Algorithm，安全散列算法）；
   - 构建时间；
   - 设备型号标识；
   - iOS 版本；
7. 导出 `build-info.json`；
8. 构建产物包含 IPA、日志、SHA-256 校验值和简短安装说明；
9. 提交一个可见的小改动，再完整重复一次构建与覆盖安装。

P1 验收清单：

- [ ] 两个独立 commit 均构建成功；
- [ ] 两个 IPA 均能通过 SideStore 安装或覆盖更新；
- [ ] Bundle ID（Bundle Identifier，应用包标识符）保持不变；
- [ ] 覆盖更新后 App 数据不丢失；
- [ ] App 显示的 commit 与构建记录一致；
- [ ] 用户能从真机导出并上传 `build-info.json`；
- [ ] 仓库不包含任何账号、证书、令牌、配对文件或私人媒体。

若 P1 未通过，不实现任何 VideoToolbox 功能。
...
- [x] 产品定位已明确；
- [x] 免费构建与安装路线已明确；
- [x] 接受 SideStore 首次使用电脑初始化；
- [x] 阶段门与证据等级已定义；
- [~] SideStore：用户正在目标 iPhone 上安装和初始化；
- [~] GitHub 私有仓库：拟创建 `bxwllzz/VideoToolbox-Studio`；
- [x] 目标 iPhone 型号：iPhone 17 Pro；
- [x] 准确 iOS 版本：iOS 26.5.2；
- [x] App 显示名称：`VideoToolbox Studio`；
- [x] Bundle ID：`io.github.bxwllzz.VideoToolboxStudio`；
- [~] P1 工程与工作流：实施中。

## 13. 接手后的第一条回复

先确认以下四项，不要直接开始写 VideoToolbox：

1. SideStore 是否已经完成初始化并能成功刷新；
2. GitHub 私有仓库地址，以及当前环境是否已获得访问权限；
3. 目标 iPhone 型号和准确 iOS 版本；
4. 用户希望使用的 App 显示名称；若未指定，暂用 `VideoToolbox Studio`。

Bundle ID 已根据 GitHub 用户名固定为 `io.github.bxwllzz.VideoToolboxStudio`，P1 两次安装验证期间不得更改。

若仓库插件尚未安装，应先建议安装 GitHub 插件；得到用户确认并成功连接后，再进行仓库操作。
